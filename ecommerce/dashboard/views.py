...
def save_subs(request, form, customer):
    ...
    # customer.activation = ACTIVE_USER  # activate customer
...
def dashboard_view(request):
    ...
    active_users = Customer.objects.filter(activation=ACTIVE_USER)
    pending_users = Customer.objects.filter(activation=PENDING_USER)
    expired_users = Customer.objects.filter(activation=EXPIRED_USER)
